...
# become the temperature at the top of layer i + 1.
  ...
  T0.C <- 20  # temperature at the surface of layer 1 [°C]
...
# Calculate Tl[i], the temperature within a layer.
  ...
  # Calculate Tl[i].
  for (i in 1:n) {
    Tl[[i]] <- sapply(z[i], function(x) TD[i] +
                   ((Q[i] + (A[i] * D[i])) / k[i]) * x -
                   ((A[i] * (x ^ 2)) / (2 * k[i])))
...
# Combine solutions into one data frame.
  ...
  #   for (i in 2:n) {
  #   z.true[[i]] <- sapply(z[i], function(x) x + D[i-1])
  # }
...
# PLOT T vs. z:
  ...
  ggplot() +
    ...
    labs(title = "Steady state geotherm: multilayer model",  # Title plot.
